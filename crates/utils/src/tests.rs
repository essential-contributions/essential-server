...
use test_utils::{
    intent_with_decision_variables, keypair, sign_corrupted, sign_with_random_keypair,
};

#[test]
fn test_serialize_intent() {
    let serialization = serialize(&intent_with_decision_variables(1));
...
#[test]
fn test_hash_intent() {
    let hash = hash(&intent_with_decision_variables(1));
...
#[test]
fn test_sign_intent() {
    let signed = sign(intent_with_decision_variables(1), keypair([0xcd; 32]).0);
...
fn test_recover() {
    ...
    let data = intent_with_decision_variables(1);
