...
async fn test_counter() {
    ...
    let solution = counter_solution(intent_address.clone(), 1, 1).await;
    ...
    let mutation_key = solution.state_mutations[0].mutations[0].key;
...
fn constraint_error_intent(decision_variables: u32) -> Intent {
    ...
    .collect()];
    intent.directive = Directive::Maximize(
        essential_constraint_vm::asm::to_bytes(vec![
            essential_constraint_vm::asm::Stack::Push(0).into(),
            essential_constraint_vm::asm::Stack::Push(1).into(),
            essential_constraint_vm::asm::Pred::Eq.into(),
        ])
...
fn state_read_error_intent(decision_variables: u32) -> Intent {
    let mut intent = Intent::empty();
    intent.slots = Slots {
        decision_variables,
        state: vec![StateSlot {
